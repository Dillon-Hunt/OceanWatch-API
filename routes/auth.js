...
router.post('/login/', async (req, res, next) => {
    ...
    const response = await query_database(`SELECT first_name, last_name, email FROM users WHERE username = '${user.username}' AND password = '${user.password}'`)
    ...
    if (response.error !== null) res.status(400).send(response.error)
        ...
        const response = await query_database(`UPDATE users SET verification_code = '${verification_code}' WHERE username = '${user.username}'`)
        if (response.error !== null) res.status(400).send(response.error)
        else res.status(200).send({ message: 'Login successful.' })
    }
})
...
router.post('/verification/', async (req, res, next) => {
    ...
    if (!valid_input(req.body.username, req.body.password, req.body.verification_code)) res.status(400).send({ message: 'Invalid input.' })
    ...
    const response = await query_database(`SELECT verified FROM users WHERE username = '${user.username}' AND password = '${user.password}' AND verification_code = '${user.verification_code}'`)
    ...
    if (response.error !== null) {
        ...
        if (response.error !== null) res.status(400).send(response.error)
        else res.status(200).send({ token: token, verified: user.verified })
    }
})
